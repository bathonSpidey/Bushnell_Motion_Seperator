...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 14:50:04 2019
...
#    new_files=new_files.remove(search_dir+"triggered.JPG")
...
# Read the template 
        template = cv2.imread(search_dir+"triggered.JPG",cv2.IMREAD_GRAYSCALE)
...
#    cv2.rectangle(img_rgb, pt, (pt[0] + w, pt[1] + h), (0,255,255), 2) 
        ...
        if len(a)==0:
            print('No Action!')
            shutil.move(file, search_dir+'no_action')
        else:
            print('Action')
            shutil.move(file, search_dir+'action')
